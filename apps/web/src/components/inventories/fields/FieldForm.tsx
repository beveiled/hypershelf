...
import type { FieldType, ValueType } from "@hypershelf/convex/schema";
import type {
  EditableKey,
  ExtraRootKeys,
  NonSystemKeys,
...
import { fieldProps, getFieldProps } from "@hypershelf/ui/fieldProps";
import { getPropsForType } from "@hypershelf/ui/fieldTypes";
import {
  AlertDialog,
  AlertDialogContent,
  AlertDialogHeader,
...
export function FieldForm({
    ...
    <K extends EditableKey>(key: K, value: ValueType) => {
      setValues((prev) => {
        ...
        if (isExtra) {
          ...
          if (child) {
            const current = (
              prev.extra?.[parent] as Record<string, ValueType> | undefined
            )?.[child];
  ...
  return (
    <motion.form
      className={cn("gap-4 px-2 md:grid-cols-2 grid grid-cols-1", className)}
...
export function NewFieldForm() {
  ...
  return (
    <>
      <Button
        variant="outline"
        className="w-lg sm:w-full mx-auto"
        onClick={() => setOpen(true)}
      >
        <Plus />
        Создать новое поле
      </Button>
      <AlertDialog open={open} onOpenChange={() => null}>
        <AlertDialogContent className="sm:!max-w-2xl max-h-[calc(100vh-6rem)] max-w-[calc(100vw-1rem)] overflow-y-auto">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-2xl font-extrabold relative text-left font-title">
              New Field
              <div className="bottom-0 left-0 h-1 w-8 absolute bg-brand"></div>
            </AlertDialogTitle>
          </AlertDialogHeader>
